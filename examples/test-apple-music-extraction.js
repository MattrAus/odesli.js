...
console.log('🎯 Summary:');
const appleMusicUrls = testUrls.filter(url => odesli.detectPlatform(url) === 'appleMusic');
const successfulExtractions = appleMusicUrls.filter(url => odesli.extractId(url));
...
console.log(`Success rate: ${((successfulExtractions.length / appleMusicUrls.length) * 100).toFixed(1)}%`);
