...
module.exports = class Odesli {
  ...
  /**
   * Fetch multiple URLs in batches with enhanced error handling
   *
   * @param {Array<string>} urls - Array of URLs to fetch
   * @param {Object} options - Batch options
   * @param {number} [options.concurrency=5] - Maximum concurrent requests
   * @param {string} [options.country='US'] - Country code for all requests
   * @param {boolean} [options.skipCache=false] - Skip cache for all requests
   * @param {number} [options.timeout] - Timeout for all requests
   * @returns {Promise<Array<Object>>} Array of results with enhanced error information
   *
   * @example
   * ```javascript
   * const results = await odesli.fetchBatch([
   *   'https://open.spotify.com/track/123',
   *   'https://music.apple.com/us/album/test/456?i=789'
   * ], { concurrency: 3, country: 'GB' });
   * 
   ...
   *     console.log(`  ID: ${result.extractedId || 'Not found'}`);
   *   }
   * });
   * ```
   */
  ...
  detectPlatform (url) {
    ...
    if (urlLower.includes('music.apple.com') || urlLower.includes('itunes.apple.com')) return 'appleMusic'
    if (urlLower.includes('youtube.com') || urlLower.includes('youtu.be')) return 'youtube'
    if (urlLower.includes('youtube.com/music') || urlLower.includes('music.youtube.com')) return 'youtubeMusic'
    if (urlLower.includes('tidal.com')) return 'tidal'
    if (urlLower.includes('deezer.com')) return 'deezer'
    if (urlLower.includes('amazon.com/music') || urlLower.includes('music.amazon.com')) return 'amazonMusic'
    if (urlLower.includes('soundcloud.com')) return 'soundcloud'
    if (urlLower.includes('pandora.com')) return 'pandora'
    if (urlLower.includes('napster.com')) return 'napster'
    if (urlLower.includes('yandex.ru/music') || urlLower.includes('music.yandex.ru')) return 'yandex'
    if (urlLower.includes('spinrilla.com')) return 'spinrilla'
